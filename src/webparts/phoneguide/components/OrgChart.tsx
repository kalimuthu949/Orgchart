...
export const OrgChart: React.FunctionComponent<IPhoneguideProps> = (
  ...
  const onRenderPlainCard = (): JSX.Element => 
  {
    ...
    return (
      <div className={classNames.plainCard}>
        ...
        <Persona className="treeview-person" {...hoverCard.current['props'].itemID} size={PersonaSize.size48}/>
        <div>
            <Icon iconName="Chat" />
            <Icon iconName="Phone"/>
        </div>
  ...
  async function getcurrentuser() {
    await graph.me.get().then(function (data) {
  ...
  async function getallusers() {
    await graph.users.top(999).get().then(function (data) {
      console.log(data);
      const users = [];
  ...
  async function getManager(userID) {
    ...
    await graph.users.getById(userID).manager().then(function (data: any) {
      if (data) {
        const userdetails = [];
        userdetails.push({
          imageUrl: "/_layouts/15/userphoto.aspx?size=L&username=" + data.mail,
  ...
  async function getDirectreports(userID) {
    ...
    await graph.users.getById(userID).directReports().then(function (data: any) {
  ...
  async function getSelecteduser(userDetails) {
    ...
    if (userDetails.length>0) {
      for (let i = 0; i < peopleList.length; i++) {
        ...
        {
          users.push({
            imageUrl: "/_layouts/15/userphoto.aspx?size=L&username=" + peopleList[i].Email,
            ID: peopleList[i].ID,
            Manager: "",
  ...
  return (<div>
    <div className="searchDiv">
      <div className="clsBack" onClick={()=>
      {
          getManager(ManagerList[0].ID);
          HidePopup();
      }}><li><Icon iconName="NavigateBack" title="Back"/></li></div>
      <div>
      <NormalPeoplePicker
    ...
    <div className="App">
      <div className="Manager">
            ...
            <div key={item.ID} className="treeview-parent">
              <Stack className="clsPersons">
                ...
                <a href="#" className="treeview-link">
                    <Persona className="treeview-person"
                      {...item}
                  ...
                  <a href="#" className="icon-link">
                    <Icon iconName="ChevronRight" onClick={() => { 
                      const ItemID=item.ID;
                      setSelectedPerson([{...item}]);
           setCallLink("https://teams.microsoft.com/_#/conversations/19:"+userID+"_"+ItemID+"@unq.gbl.spaces?ctx=chat");
                      ShowPopup(); }} />
                  </a>
                  </div>
                  </div>
        ...
        <div className="Reportees">
          ...
          <label>No. of reporting person to <b>{ManagerList.length>0?ManagerList[0].text:""}</b> ({ReporteeList.length})</label>
    ...
    {SelectedPerson.length>0?<div  id="myPopup" className="clsPopup">
    <div className="clsFirstDiv">
    <div className="clsPersonDetails">
      <Persona className="treeview-person" {...SelectedPerson[0]} size={PersonaSize.size56}/>
    </div>
    <div className="clsClose"><li><Icon iconName="ChromeClose" onClick={()=>{
      setSelectedPerson([]);
      HidePopup();
    }}/></li></div>
    </div>
    <div className="clsIcons">
        <li><a href={CallLink} target="_blank" rel="noopener noreferrer"><Icon iconName="Chat" title="Chat"/></a></li>
        <li><a href={"mailto:"+SelectedPerson[0].Email} target="_blank" rel="noopener noreferrer"><Icon iconName="Mail" title="Mail"/></a></li>
        <li><a href={CallLink} target="_blank" rel="noopener noreferrer"><Icon iconName="Phone" title="Phone"/></a></li>
    </div>
    ...
    <div className="clsContacts">
      <h3><b>Contact</b></h3>
      <div>{SelectedPerson[0].mobilePhone?SelectedPerson[0].mobilePhone:"N/A"}</div>
    </div>
    </div>:<div  id="myPopup" className="clsPopup">
    <div className="clsFirstDiv">
    <div className="clsPersonDetails">
      <Persona className="treeview-person" {...SelectedPerson[0]} size={PersonaSize.size56}/>
    </div>
    ...
    <div className="clsIcons">
        <li><Icon iconName="Chat" title="Chat"/></li>
        <li><Icon iconName="Mail" title="Mail"/></li>
        <li><Icon iconName="Phone" title="Phone"/></li>
    </div>
    <div className="clsEmail">
      <h3><b>Email</b></h3>
